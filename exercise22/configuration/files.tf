resource "local_file" "cloud_init" {
  ...
  content = templatefile("./template/cloud_init.yml", {
    ...
    volume_device       = hcloud_volume.volume_exercise_22.linux_device
    ssl_private         = local_file.tls_private_key.content
    ssl_certificate     = local_file.tls_certificate_key.content
    nginx_configuration = file("./template/nginx_default")
  })
  depends_on = [local_file.tls_private_key, local_file.tls_certificate_key]
}
